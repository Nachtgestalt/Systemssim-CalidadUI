...
export class ProcesosespecialesoperacionesComponent implements OnInit {
  ...
  GetOperacionProcesosEspeciales() {
    ...
    $.ajax({
      url: Globals.UriRioSulApi + 'ProcesosEspeciales/ObtieneOperacionProcesosEspeciales' + _request,
      dataType: 'json',
        ...
        if (json.Message.IsSuccessStatusCode) {
          ...
          for (let i = 0; i < json.Vst_ProcesosEspeciales.length; i++) {
            ...
            if (json.Vst_ProcesosEspeciales[i].Activo === true) {
              // tslint:disable-next-line:max-line-length
              sOptions += '<td style="text-align:center"><a onclick="SetId(' + json.Vst_ProcesosEspeciales[i].ID + ');" class="waves-effect waves-light btn tooltiped modal-trigger" data-target="modalEnableOperacionProcesosEspeciales" data-tooltiped="Activa / Inactiva la operación seleccionada"><strong><u>Inactivar</u></strong></a></td>';
            } else {
              // tslint:disable-next-line:max-line-length
              sOptions += '<td style="text-align:center"><a onclick="SetId(' + json.Vst_ProcesosEspeciales[i].ID + ');" class="waves-effect waves-light btn tooltiped modal-trigger" data-target="modalEnableOperacionProcesosEspeciales" data-tooltiped="Activa / Inactiva la operación seleccionada"><strong><u>Activar</u></strong></a></td>';
            }
            sOptions += '</tr>';
            index ++;
          }
