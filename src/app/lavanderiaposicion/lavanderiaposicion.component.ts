...
export class LavanderiaposicionComponent implements OnInit {
  ...
  GetPosicionLavanderia() {
    ...
    // }
    $.ajax({
      url: Globals.UriRioSulApi + 'Lavanderia/PosicionLavanderia' + _request,
      dataType: 'json',
      ...
      success: function (json) {
        if (json.Message.IsSuccessStatusCode) {
          ...
          for (let i = 0; i < json.Vst_Lavanderia.length; i++) {
            ...
            sOptions += '<td><a onclick="javascript: SetId(' + json.Vst_Lavanderia[i].ID + '); DisposeEditPosicionCortador(); GetInfoPosicion();" class="waves-effect waves-light btn tooltipped modal-trigger" data-target="modalEditPosicionCortador" data-position="bottom" data-tooltip="Edita el defecto  seleccionado"><i class="material-icons right">edit</i></a></td>';
            ...
            if (json.Vst_Lavanderia[i].Activo === true) {
              // tslint:disable-next-line:max-line-length
              sOptions += '<td style="text-align:center"><a onclick="SetId(' + json.Vst_Lavanderia[i].ID + ');" class="waves-effect waves-light btn tooltiped modal-trigger" data-target="modalEnablePosicionProcesosEspeciales" data-tooltiped="Activa / Inactiva la posición del cortador seleccionado"><strong><u>Inactivar</u></strong></a></td>';
            } else {
              // tslint:disable-next-line:max-line-length
              sOptions += '<td style="text-align:center"><a onclick="SetId(' + json.Vst_Lavanderia[i].ID + ');" class="waves-effect waves-light btn tooltiped modal-trigger" data-target="modalEnablePosicionProcesosEspeciales" data-tooltiped="Activa / Inactiva la posición del cortador seleccionado"><strong><u>Activar</u></strong></a></td>';
            }
            sOptions += '</tr>';
            index ++;
          }
  ...
  NewPosicionProcesosEspeciales() {
    if ($('#CVE_NEW_CORTADOR').val() === '') {
      ...
      // if (Result) {
        ...
        $.ajax({
          url: Globals.UriRioSulApi + 'Lavanderia/PosicionLavanderia',
          type: 'POST',
