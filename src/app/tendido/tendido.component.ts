      ...
      success: function (json) {
        if (json.Message.IsSuccessStatusCode) {
          ...
          for (let i = 0; i < json.Vst_Cortadores.length; i++) {
            ...
            sOptions += '<td><a onclick="javascript: SetId(' + json.Vst_Cortadores[i].ID + '); DisposeEditTendido(); GetInfoTendido();" class="waves-effect waves-light btn tooltipped modal-trigger" data-target="modalEditTendido" data-position="bottom" data-tooltip="Edita el tendido seleccionado"><i class="material-icons right">edit</i></a></td>';
