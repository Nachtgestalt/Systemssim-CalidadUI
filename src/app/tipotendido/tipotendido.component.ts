      ...
      success: function (json) {
        if (json.Message.IsSuccessStatusCode) {
          ...
          for (let i = 0; i < json.Vst_Cortadores.length; i++) {
            ...
            sOptions += '<td><a onclick="SetId(' + json.Vst_Cortadores[i].ID + '); DisposeEditTipoTendido(); GetInfoTipoTendido();" class="waves-effect waves-light btn tooltipped modal-trigger" data-target="modalEditTipoTendido" data-position="bottom" data-tooltip="Edita el tipo tendido seleccionado"><i class="material-icons right">edit</i></a></td>';
